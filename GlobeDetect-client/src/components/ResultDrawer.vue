<template>
<el-drawer v-model="showResultDrawer" size="500" :show-close="false">
	<template #header>
	...
	<div class="resultDrawerBody">
		...
		<el-table ref="resultTableRef" :data="resultInfo" v-show="!showEmpty" table-layout="auto">
...
const resultInfoStore = useResultInfoStore()
const { resultInfo } = storeToRefs(resultInfoStore)
const { setLayerView } = resultInfoStore

const showResultDrawer = ref(false)

// open/fold result drawer
const toggleResultDrawer = () => showResultDrawer.value = !showResultDrawer.value

// show empty icon
const showEmpty = computed(() => resultInfo.value.length === 0)
...
const locateLayer = (index) => {
	...
	map.value.fitBounds([ southWest, northEast ])
}
