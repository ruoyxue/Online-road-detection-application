...
function isTileInSelection(tileRect) {
	...
	var areaPolygon = draw.value.getAll().features[0];
...
function getBounds() {
	const { draw } = storeToRefs(useMapboxStore())
	let coordinates = draw.value.getAll().features[0].geometry.coordinates[0]
...
export function previewGrid(max_grid){
	const { draw, map } = storeToRefs(useMapboxStore())
	const { zoom } = storeToRefs(useBasicSettingsStore())
	if (draw.value.getAll().features.length > 0) {
		let grid = getGrid(zoom.value, max_grid)
		if (grid === null) {
			return null
		}
